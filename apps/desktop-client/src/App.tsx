  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        <div className="brand">
          <div className="app-icon" aria-hidden="true"><span>‹›</span></div>
          <div>
            <p className="eyebrow">{t("Workspace console")}</p>
            <h1>{t("MCP Desktop")}</h1>
          </div>
        </div>
        <p className="sidebar-copy">{t("Manage local runtimes and secure public access.")}</p>
        ...
        <nav className="workspace-list" aria-label={t("Workspace")}>
          ...
          <>
            <header className="content-header">
              ...
              <div className="header-actions">
                ...
                <button className="button secondary quick-action" onClick={() => void copy(currentAuthorizationCode)} title={t("Copy authorization code")}>⌘ {t("Copy auth code")}</button>
            ...
            {activeTab === "configuration" && <div className="dashboard-grid" role="tabpanel">
              <section className="card workspace-card">
                ...
                <div className="form-grid">
                  <Field label={t("Name")}>
                    <input value={selected.name} onChange={(event) => updateSelected((profile) => ({ ...profile, name: event.target.value }))} />
                  </Field>
                    ...
                    <>
                      ...
                      {selected.tunnel.cloudflare_mode === "named" && (
                        <>
                          ...
                          <SecretField label={t("Tunnel token")} value={selected.tunnel.cloudflare_token} shown={reveal.cloudflare} onToggle={() => setReveal((state) => ({ ...state, cloudflare: !state.cloudflare }))} onChange={(value) => updateSelected((profile) => ({ ...profile, tunnel: { ...profile.tunnel, cloudflare_token: value } }))} t={t} />
...
function SecretField({ label, value, shown, onToggle, onChange, copyLabel, onCopy, t }: { label: string; value: string; shown?: boolean; onToggle: () => void; onChange: (value: string) => void; copyLabel?: string; onCopy?: () => void; t: (text: string) => string }) {
  return <Field label={label} wide><div className="secret-input"><input type={shown ? "text" : "password"} value={value} onChange={(event) => onChange(event.target.value)} /><button type="button" onClick={onToggle}>{t(shown ? "Hide" : "Reveal")}</button>{onCopy && <button type="button" onClick={onCopy}>{copyLabel}</button>}</div></Field>;
}
