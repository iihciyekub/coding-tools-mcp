...
function App() {
  ...
  useEffect(() => { setRevealCredential(false); setConfirmDelete(false); setRuntimeLogs(null); }, [selectedId]);
  useEffect(() => {
  ...
  const toggleWorkspace = async () => {
    ...
    try {
      ...
      if (running) next = await api.stopProfile(selected.id);
      else {
