...
desktop-check:
	cd apps/desktop-client && npm run check
	cd apps/desktop-client/src-tauri && cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test

desktop-build:
	cd apps/desktop-client && npm run tauri build
