...
CATEGORIES: dict[str, ToolCategory] = {
    ...
    "code": ToolCategory("Code navigation / 代码定位", "Use code_definition/code_references as the normal entry points; when a file position is supplied they prefer LSP semantics and fall back safely. Use explicit lsp_* tools for semantic debugging or direct position-based control.", ("代码", "符号", "定义", "引用", "语义", "lsp", "symbol")),
...
_GROUPS: dict[str, tuple[tuple[str, str, tuple[str, ...]], ...]] = {
    ...
    "code": (
        ...
        ("code_definition", "Normal definition lookup: symbol scan without a position, or LSP-first semantic lookup when file/line/column is supplied; LSP runtime failures fall back safely.", ("查找定义", "函数定义", "find definition")),
        ("code_references", "Normal reference lookup: exact identifier scan without a position, or LSP-first semantic references when file/line/column is supplied; LSP runtime failures fall back safely.", ("查找引用", "标识符引用", "find references")),
        ("lsp_status", "Check Python, TypeScript/JavaScript, Rust, or Swift/SourceKit-LSP availability, resolved command, project root, and process state.", ("语言服务器", "lsp状态", "language server status", "sourcekit")),
    ...
    "project": (
        ("workspace_overview", "Orient in an unfamiliar project before choosing narrower paths; Apple projects also expose bounded Xcode/Swift/SDK/SourceKit metadata without starting Xcode.", ("项目概览", "工作区概览", "了解项目", "workspace overview", "xcode overview", "swift project")),
    ...
    "checks": (
        ("checks_discover", "Find existing project-defined checks and, by default, rank them from current Git changes with explicit recommendation reasons; no commands are invented or run.", ("发现测试", "测试入口", "检查入口", "推荐测试", "discover tests", "find checks", "targeted checks")),
    ...
    "runtime": (
        ...
        ("runtime_doctor", "Diagnose missing commands, aliases, environment issues, LSP state, and macOS Apple toolchain availability instead of repeated trial-and-error execution.", ("环境诊断", "环境检查", "缺少命令", "xcode环境", "swift环境", "runtime doctor")),
